...
export default function LiveData({ initialData }: Props) {
  ...
  return (
    <div className="flex items-center justify-between py-4 mb-2" style={{ borderBottom: "1px solid #E5E5E5", paddingBottom: "16px" }}>
      <div className="flex items-center gap-4">
        {autoRefresh && <span className="live-dot" />}
        <span className="text-meta">
          {autoRefresh ? "LIVE" : "PAUSED"} — Last updated: {formatTime(lastUpdated)}
        </span>
      </div>
      <button
